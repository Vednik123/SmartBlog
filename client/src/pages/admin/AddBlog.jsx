...
setLoading(false);
  ...
  useEffect(()=>{
    // initiate quill editor for blog content and description where we can add images and text and also change the font size. We installed it using npm install quill
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className="bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded">
        ...
        <input type="text" value={title} placeholder='Type here' onChange={(e)=>setTitle(e.target.value)} required className='w-full max-w-lg mt-2 p-2 border border-gray-300 outline-none rounded'/>

          <p className='mt-4'>Sub Title</p>
        <input type="text" value={subTitle} placeholder='Type here' onChange={(e)=>setSubTitle(e.target.value)} required className='w-full max-w-lg mt-2 p-2 border border-gray-300 outline-none rounded'/>
